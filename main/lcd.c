...
#ifndef APP_LCD_RGB_ELEMENT_ORDER
#define APP_LCD_RGB_ELEMENT_ORDER            LCD_RGB_ELEMENT_ORDER_RGB
#endif
...
void lcd_init(void)
{
    ...
    esp_err_t i2c_err = i2c_master_get_bus_handle(BSP_I2C_NUM, &i2c_handle);
    if (i2c_err == ESP_OK) {
        ESP_LOGI(TAG, "Reuse existing I2C bus %d for touch", BSP_I2C_NUM);
    } else {
        ...
        ESP_LOGI(TAG, "Created I2C bus %d for touch", BSP_I2C_NUM);
    }
    ...
    esp_lcd_dpi_panel_config_t dpi_config ={
        ...
        .num_fbs = APP_LCD_DPI_BUFFER_NUMS,                                 
        .video_timing = {                             
            .h_size = 480,                            
            .v_size = 800,                            
            .hsync_back_porch = 42,                   
    ...
    const esp_lcd_panel_dev_config_t panel_config = {
        ...
        .rgb_ele_order = APP_LCD_RGB_ELEMENT_ORDER,
        .bits_per_pixel = 16,
        .vendor_config = &vendor_config,
    };
    ESP_ERROR_CHECK(esp_lcd_new_panel_st7701(io, &panel_config, &disp_panel));
    s_disp_panel = disp_panel;
    esp_lcd_panel_reset(disp_panel);
    esp_lcd_panel_init(disp_panel);

#if APP_LCD_TOUCH_ENABLE
    ESP_LOGI(TAG, "Initialize GT911 touch controller, addr=0x%02X", APP_LCD_TOUCH_I2C_ADDR);
    esp_lcd_panel_io_handle_t tp_io_handle = NULL;
    esp_lcd_touch_handle_t tp_handle = NULL;
    esp_lcd_panel_io_i2c_config_t tp_io_config = ESP_LCD_TOUCH_IO_I2C_GT911_CONFIG();
    tp_io_config.scl_speed_hz = 100000;
    tp_io_config.dev_addr = APP_LCD_TOUCH_I2C_ADDR;
    esp_err_t touch_err = esp_lcd_new_panel_io_i2c(i2c_handle, &tp_io_config, &tp_io_handle);
    if (touch_err == ESP_OK) {
        ...
        const esp_lcd_touch_config_t tp_cfg = {
            ...
            .flags = {
                .swap_xy = APP_LCD_TOUCH_SWAP_XY,
                .mirror_x = APP_LCD_TOUCH_MIRROR_X,
                .mirror_y = APP_LCD_TOUCH_MIRROR_Y,
            },
    ...
    if (touch_err == ESP_OK) {
        ...
        ESP_LOGW(TAG, "GT911 touch init failed: %s", esp_err_to_name(touch_err));
    }
#else
    ESP_LOGW(TAG, "APP_LCD_TOUCH_ENABLE=0, skip touch init");
#endif
...
{
    if (!s_disp_panel || !color_data) {
        return ESP_ERR_INVALID_STATE;
    }
    return esp_lcd_panel_draw_bitmap(s_disp_panel, x_start, y_start, x_end, y_end, color_data);
}
