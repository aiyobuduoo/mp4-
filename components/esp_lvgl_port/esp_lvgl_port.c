...
/*
 * MIPI DSI DBI panel IO in ESP-IDF 5.4 does not provide register_event_callbacks.
 * Calling esp_lcd_panel_io_register_event_callbacks() on that IO jumps through a
 * NULL function pointer. This project uses esp_lcd_dpi_panel_register_event_callbacks()
 * on the DPI panel handle instead.
 */
#if 1
#define LVGL_PORT_HANDLE_FLUSH_READY 0
#elif (ESP_IDF_VERSION < ESP_IDF_VERSION_VAL(4, 4, 4)) || (ESP_IDF_VERSION == ESP_IDF_VERSION_VAL(5, 0, 0))
#define LVGL_PORT_HANDLE_FLUSH_READY 0
#else
...
typedef struct {
    ...
    bool                      async_flush_ready;
    lvgl_port_rotation_cfg_t  rotation;     /* Default values of the screen rotation */
    lv_disp_drv_t             disp_drv;     /* LVGL display driver */
    lv_color_t               *bounce_buf;   /* Internal DMA buffer used before handing pixels to DPI */
    uint32_t                  bounce_pixels;
...
#endif
static bool IRAM_ATTR lvgl_port_dpi_flush_ready_callback(esp_lcd_panel_handle_t panel, esp_lcd_dpi_panel_event_data_t *edata, void *user_ctx);
...
lv_disp_t *lvgl_port_add_disp(const lvgl_port_display_cfg_t *disp_cfg)
{
    ...
    /* Display context */
    lvgl_port_display_ctx_t *disp_ctx = heap_caps_calloc(1, sizeof(lvgl_port_display_ctx_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    ...
    disp_ctx->async_flush_ready = false;
    ...
    bounce_buf = heap_caps_aligned_alloc(64,
                                         disp_cfg->buffer_size * sizeof(lv_color_t),
                                         MALLOC_CAP_INTERNAL | MALLOC_CAP_DMA | MALLOC_CAP_8BIT);
    ESP_GOTO_ON_FALSE(bounce_buf, ESP_ERR_NO_MEM, err, TAG, "Not enough memory for LVGL DPI bounce buffer allocation!");
    disp_ctx->bounce_buf = bounce_buf;
    disp_ctx->bounce_pixels = disp_cfg->buffer_size;
    ...
    const esp_lcd_dpi_panel_event_callbacks_t dpi_cbs = {
        .on_color_trans_done = lvgl_port_dpi_flush_ready_callback,
    };
    esp_err_t cb_ret = esp_lcd_dpi_panel_register_event_callbacks(disp_ctx->panel_handle, &dpi_cbs, &disp_ctx->disp_drv);
    if (cb_ret == ESP_OK) {
        disp_ctx->async_flush_ready = true;
    } else {
        ESP_LOGW(TAG, "DPI flush callback unavailable (%s), fallback to immediate flush ready", esp_err_to_name(cb_ret));
    }
...
esp_err_t lvgl_port_remove_disp(lv_disp_t *disp)
{
    ...
    if (disp_ctx && disp_ctx->bounce_buf) {
        free(disp_ctx->bounce_buf);
        disp_ctx->bounce_buf = NULL;
    }
    free(disp_ctx);
...
static void lvgl_port_flush_callback(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    size_t pixels = (size_t)(offsetx2 - offsetx1 + 1) * (size_t)(offsety2 - offsety1 + 1);
    const void *draw_buf = color_map;
    if (disp_ctx->bounce_buf && pixels <= disp_ctx->bounce_pixels) {
        memcpy(disp_ctx->bounce_buf, color_map, pixels * sizeof(lv_color_t));
        draw_buf = disp_ctx->bounce_buf;
    }

    // copy a buffer's content to a specific area of the display
    esp_err_t err = esp_lcd_panel_draw_bitmap(disp_ctx->panel_handle, offsetx1, offsety1, offsetx2 + 1, offsety2 + 1, draw_buf);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "LCD draw bitmap failed: %s", esp_err_to_name(err));
        lv_disp_flush_ready(drv);
        return;
    }

    if (!disp_ctx->async_flush_ready) {
        lv_disp_flush_ready(drv);
    }
}
